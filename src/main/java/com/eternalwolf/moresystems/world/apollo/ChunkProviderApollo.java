...
import com.eternalwolf.moresystems.world.ChunkProviderBase;
...
{
    public static final IBlockState BLOCK_TOP = ModBlocks.APOLLO_DUST.getDefaultState();
    public static final IBlockState BLOCK_FILL = ModBlocks.APOLLO_ROCK.getDefaultState();
    public static final IBlockState BLOCK_LOWER = ModBlocks.APOLLO_STONE.getDefaultState();
    ...
    private static final int CRATER_PROB = 150;
    ...
    public ChunkProviderApollo(World par1World, long par2, boolean par4)
    {
    ...
    public void setBlocksInChunk(int chunkX, int chunkZ, ChunkPrimer primer)
    {
        ...
        for (int x = 0; x < ChunkProviderApollo.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < ChunkProviderApollo.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < ChunkProviderApollo.CHUNK_SIZE_Y; y++)
                {
                    if (y < ChunkProviderApollo.MID_HEIGHT + yDev)
                    {
        ...
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
        {
            for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
            {
                for (int x = 0; x < ChunkProviderApollo.CHUNK_SIZE_X; x++)
                {
                    for (int z = 0; z < ChunkProviderApollo.CHUNK_SIZE_Z; z++)
                    {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderApollo.CHUNK_SIZE_X + x, cz * ChunkProviderApollo.CHUNK_SIZE_Z + z) / ChunkProviderApollo.CRATER_PROB)
                        {
